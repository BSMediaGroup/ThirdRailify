...
- `/goats` — approved/published search, URL-backed filters, bounded gallery pagination, a MapLibre raster basemap with accessible screen-space-clustered DOM pins, and an accessible list fallback.
...
Configure the same `THIRDRAILIFY_COMMUNITY_API_SECRET` as an encrypted Pages secret in Public and Admin. It must never be a `VITE_` variable. A build-time `VITE_GOATS_MAP_STYLE_URL` may override the licensed MapLibre-compatible style. When Vite receives no build-time override, Public uses its explicit OpenFreeMap Natural Earth raster style; a Wrangler runtime variable alone is not treated as a Vite build variable. Approved locations are rendered as DOM pins grouped in deterministic screen-space buckets, avoiding MapLibre's GeoJSON worker while retaining cluster expansion, keyboard-accessible selection, and the list fallback. No Mapbox token is embedded.
