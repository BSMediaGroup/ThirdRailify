...
Configure the same `THIRDRAILIFY_COMMUNITY_API_SECRET` as an encrypted Pages secret in Public and Admin. It must never be a `VITE_` variable. `VITE_GOATS_MAP_STYLE_URL` is an optional non-secret Public build variable for a licensed MapLibre style. With it blank, the feature uses a basemap-free local/test style while pins and the list remain usable; no demo tile service or Mapbox token is embedded.
