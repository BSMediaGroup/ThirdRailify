...
Each direct SVG marker opens a dark, keyboard-reachable approved-listing card containing safe text-created DOM, optional approved imagery, an SVG country flag, approximate location, excerpt, and a first-party detail link. SVG flags also prefix the selected panel, gallery cards, detail identity, shared-point/fallback selectors, and hero city tags. The same marker/list selection contract remains authoritative. The active engine can expand in place into a modal-scale viewport without constructing a second map: body scrolling is locked, Escape and the visible close control restore the embedded state, and the instance is resized/recentered so live geography, in-bounds cards, pan/zoom, attribution, and selection remain intact.
