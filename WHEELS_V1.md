...
Normal `/wheels/:slug` detail pages now include a compact gallery navigator between the unchanged Back to Wheels breadcrumb and the wheel heading. Its previous/next titles and `Wheel NN / NN` position come directly from the unfiltered `listWheels("", "recent")` Public projection—the same default authoritative ordering shown by `/wheels`. The pure neighbour helper preserves that order exactly, never wraps first to last, and returns no neighbours when the current slug is absent, so hidden, archived, deleted, or privately assigned wheels cannot infer gallery adjacency. Presentation mode has no navigator.
