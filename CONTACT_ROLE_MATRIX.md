...
| `info@thirdrailify.com` | General contact and current transactional reply route | Public footer; Admin commerce profile/default templates; GOATS support links | No | Yes, through Admin `MAIL_REPLY_TO` for account and GOATS Resend messages | Yes | No |
...
No Public or Admin feature sends from the five published `@thirdrailify.com` contact addresses. No application code establishes mailbox provisioning, routing, staffing, response time, or monitoring for any address. Historical migration CSV content is not runtime policy authority.
