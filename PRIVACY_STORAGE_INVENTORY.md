...
- Account V2 adds no browser storage key: current contact data, saved delivery addresses, and order history are fetched only through authenticated same-origin requests and remain out of cookies, `localStorage`, and client caches.
...
- Commerce authority stores catalogue/variant identity, account-linked Customer identity, encrypted current contact name/optional phone, encrypted reusable delivery addresses, encrypted immutable order-time delivery snapshots, CAD totals, Stripe Checkout Session and PaymentIntent identifiers, payment/refund status, Printful mappings/identifiers, webhook evidence, and audit dates. Current saved-address deletion is supported independently of preserved historical order evidence. Normal Public checkout and fulfilment are disabled, and Third Railify does not store full card numbers.
