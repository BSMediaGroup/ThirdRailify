...
export function TurnstileWidget({ siteKey, action, resetKey, onToken, onUnavailable }: {
  siteKey: string;
  action: string;
  resetKey: number;
  ...
  useEffect(() => {
    ...
    loadTurnstile()
      ...
      .catch(() => {
        if (active) onUnavailable("Verification is unavailable. Check your connection and try again.");
      });
    return () => {
      active = false;
...
function loadTurnstile() {
  ...
  scriptPromise = new Promise<TurnstileApi>((resolve, reject) => {
    ...
    if (!existing) {
      ...
      script.async = true;
      script.defer = true;
      script.dataset.thirdrailifyTurnstile = "true";
