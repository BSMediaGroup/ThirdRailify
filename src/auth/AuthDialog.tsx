...
import { submitAuthority, validatedAuthorizationUrl } from "./client";
...
export function AuthDialog({ initialMode, initialError, resetToken, config, onClose, onSession }: {
  ...
  onClose: () => void;
  onSession: (payload: SessionPayload) => Promise<void>;
}) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const focusable = Array.from(dialog.current.querySelectorAll<HTMLElement>('button:not([disabled]),a[href],input:not([disabled]),select:not([disabled]),[tabindex]:not([tabindex="-1"])'));
  ...
  const heading = useMemo(() => {
    if (oauthProvider) return `Continue with ${oauthProvider.label}`;
  ...
  const startOAuth = async () => {
    if (!oauthProvider || !turnstileToken) {
      setError("Complete verification before continuing to the provider.");
      return;
    }
    setBusy(true);
    setError("");
    try {
      const payload = await submitAuthority(`oauth/${oauthProvider.id}/start`, {
        turnstileToken,
        returnTo: returnPath(),
      });
      window.location.assign(validatedAuthorizationUrl(String(payload.authorizationUrl || "")));
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Provider sign in could not start.");
      resetChallenge();
      setBusy(false);
    }
  };
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!turnstileToken) { setError("Complete verification before continuing."); return; }
    ...
    try {
      ...
      const payload = await submitAuthority(path, {
        email: String(values.get("email") || ""),
        displayName: String(values.get("displayName") || ""),
        password,
        token: mode === "reset" ? resetToken : undefined,
        turnstileToken,
        returnTo: returnPath(),
      });
      if (payload.verificationPending || mode === "forgot") {
        setMessage(payload.message || "Check your email to continue.");
        resetChallenge();
      } else {
    ...
    <div className="auth-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}>
      <div ref={dialog} className="auth-dialog" role="dialog" aria-modal="true" aria-labelledby="auth-title" aria-describedby="auth-intro">
        ...
        {configured && !message && oauthProvider && (
          <section className="auth-oauth-step">
            ...
            <form className="auth-form" onSubmit={submit}>
              {(mode === "signin" || mode === "signup") && (
                <div className="auth-mode-tabs" role="group" aria-label="Account mode">
                  ...
                  {config!.emailSignupConfigured && <button type="button" aria-pressed={mode === "signup"} onClick={() => switchMode("signup")}>Create account</button>}
                </div>
              )}
              ...
              {mode !== "forgot" && (
                <label className="auth-field"><span>Password</span><span className="auth-password"><input ref={mode === "reset" ? initialField : undefined} name="password" type={showPassword ? "text" : "password"} autoComplete={mode === "signin" ? "current-password" : "new-password"} minLength={mode === "signin" ? undefined : 12} maxLength={256} required /><button type="button" onClick={() => setShowPassword((value) => !value)}>{showPassword ? "Hide" : "Show"}</button></span></label>
              )}
        ...
        <footer className="auth-dialog__footer">By continuing, you agree to the <Link to="/terms" onClick={onClose}>Terms</Link> and acknowledge the <Link to="/privacy" onClick={onClose}>Privacy Policy</Link>.</footer>
      </div>
    </div>
  );
}

const AuthField = forwardRef<HTMLInputElement, { label: string; name: string } & InputHTMLAttributes<HTMLInputElement>>(function AuthField({ label, name, ...props }, ref) {
  return <label className="auth-field"><span>{label}</span><input ref={ref} name={name} required {...props} /></label>;
});

function returnPath() {
  const path = window.location.pathname;
  return path === "/account/login" ? "/account" : path.startsWith("/api/") ? "/account" : path;
}
