...
export async function uploadWheelMedia(slug: string, purpose: "background" | "centre" | "segment-fill" | "avatar", file: Blob, csrfToken: string, filename = "") { return request<{ ok: true; reused?: boolean; asset: WheelMediaAsset }>(`/api/wheels/${encodeURIComponent(slug)}/media/${purpose}`, { method: "POST", headers: { "Content-Type": file.type || "application/octet-stream", "X-CSRF-Token": csrfToken, ...(filename ? { "X-ThirdRailify-Filename": filename } : {}) }, body: file }); }
...
export async function listAccessibleWheels(search = "", scope = "mine") { return request<{ ok: true; items: AccessibleWheelSummary[]; count: number }>(`/api/wheels/stages/lookup?search=${encodeURIComponent(search)}&scope=${encodeURIComponent(scope)}`); }
