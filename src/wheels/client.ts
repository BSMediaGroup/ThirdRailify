import type { Wheel, WheelAccess, WheelMediaAsset, WheelSummary } from "./types";
...
async function request<T>(input: string, init: RequestInit = {}) { const response = await fetch(input, { ...init, headers: { Accept: "application/json", ...(init.headers || {}) }, credentials: "include", cache: "no-store" }); const payload = await response.json().catch(() => null) as (T & { error?: string; message?: string }) | null; if (!response.ok || !payload) { const error = new Error(payload?.message || "The wheel service is unavailable.") as ApiError; error.code = payload?.error; error.status = response.status; throw error; } return payload; }
