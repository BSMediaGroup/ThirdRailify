import { importedEntryIdentity, normalizeEntryIdentity } from '../lib/entrant-identity.mjs';
...
async function parseCanonical(document, sourceName, defaults) {
  ...
  if (![1, 2, WHEEL_FILE_FORMAT_VERSION].includes(document.formatVersion)) throw new Error(`Third Railify wheel format version ${document.formatVersion} is not supported.`);
  ...
  const proposal = { title: wheel.title, description: wheel.description, config, entries: wheel.entries.map((entry) => ({ id: newId(), identity: importedEntryIdentity(entry.identity), label: entry.label, ...(entry.appearance ? { appearance: portableAppearance(entry) } : {}), avatarUrl: entry.avatarUrl || null, order: entry.order, weight: entry.weight, colour: entry.color, style: entry.style ? runtimeStyle(entry.style, runtimeRefs) : null, state: entry.active ? "active" : "hidden" })), media, messages: [info("integrity", integrityStatus === "verified" ? "verified" : "absent", integrityStatus === "verified" ? "SHA-256 matched the canonical wheel payload. This detects corruption; it is not a signature." : "This supported file has no integrity hash."), info("entry identity", "fresh local IDs", "Each imported row receives a fresh ID. Entry types are retained, but imports never claim an automatic award identity or merge by name."), info("official history and access", "never imported", "Portable files contain creator-editable content only."), ...paletteMessages(wheel._paletteWarnings)], sourceIndex: 0 };
...
function normalizePortableEntry(entry, index, availableImageAssetRefs, warnings) {
  ...
  rejectUnknown(entry, new Set(["label", "identity", "avatarUrl", "appearance", "weight", "color", "style", "active", "order"]), `entry ${index + 1}`);
...
function genericEntry(row, index, messages) {
  ...
  const supported = new Set(["label", "text", "name", "weight", "color", "colour", "active", "enabled", "hidden", "order", "identity"]); const ignored = Object.keys(row).filter((key) => !supported.has(key)); if (ignored.length) messages.push(warning(`entry ${index + 1}`, "unsupported fields ignored", ignored.join(", ")));
  return { id: newId(), identity: importedEntryIdentity(normalizeEntryIdentity(row.identity)), label: boundedText(row[labels[0]], 1, WHEEL_FILE_LIMITS.entryLabel, `Entry ${index + 1} label`), order: index, weight, colour, state: active ? "active" : "hidden" };
}
...
function portableAvatar(value) { if (value == null || value === '') return null; const url = new globalThis.URL(value); if (typeof value !== 'string' || value.length > 2048 || url.protocol !== 'https:' || url.username || url.password) throw new Error('Avatar must be an HTTPS URL without embedded credentials.'); return url.href; }
