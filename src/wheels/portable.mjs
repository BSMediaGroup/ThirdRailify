...
function normalizePortableWheel(value, defaults = THIRD_RAIL_GOLD_CONFIG) {
  ...
  const entries = value.entries.map((entry, index) => normalizePortableEntry(entry, index)).sort((a, b) => a.order - b.order || a._sourceOrder - b._sourceOrder).map((entry, order) => ({ label: entry.label, weight: entry.weight, color: entry.color, style: entry.style, active: entry.active, order }));
...
function normalizePortableEntry(entry, index) {
  ...
  const style = entry.style == null ? null : normalizePortableStyle(entry.style, color || "#F3C928");
...
function normalizeSettings(value) {
  ...
  const preset = new Set(["third-rail-gold", "live-wire-red", "gina-violet", "high-voltage-mono", "signal-teal", "after-hours", "high-voltage-hazard", "rail-strike", "goated-circuit", "night-signal", "custom"]).has(value.themePreset) ? value.themePreset : "third-rail-gold";
  ...
  const paletteStyles = value.paletteStyles == null ? palette.map((color) => ({ mode: "solid", color })) : value.paletteStyles;
  if (!Array.isArray(paletteStyles) || paletteStyles.length !== palette.length) throw new Error("Palette styles must align with palette colours.");
  return { themePreset: preset, palette, paletteStyles: paletteStyles.map((style, index) => normalizePortableStyle(style, palette[index])), pointerAccent: normalizeHex(value.pointerAccent || THIRD_RAIL_GOLD_CONFIG.pointerAccent, "Pointer accent"), centreTreatment: oneOf(value.centreTreatment, ["bolt", "signal", "ring"], "bolt"), backgroundIntensity: oneOf(value.backgroundIntensity, ["low", "medium", "high"], "high"), labelContrast: oneOf(value.labelContrast, ["light", "dark"], "light"), spinDurationMs: finiteInteger(value.spinDurationMs ?? 6500, 2000, 20000, "Spin duration"), tickingSoundEnabled: value.tickingSoundEnabled !== false, spinSoundPreset: normalizeSpinSoundPreset(value.spinSoundPreset), winnerSoundEnabled: value.winnerSoundEnabled !== false, winnerSoundPreset: normalizeWinnerSoundPreset(value.winnerSoundPreset), celebrationEnabled: value.celebrationEnabled !== false, confettiEnabled: value.confettiEnabled !== false, fireworksEnabled: value.fireworksEnabled !== false, winnerLightingEnabled: value.winnerLightingEnabled !== false, celebrationIntensity: oneOf(value.celebrationIntensity, ["subtle", "normal", "strong"], "normal"), backgroundEnabled: value.backgroundEnabled !== false, backgroundFocalX: finiteInteger(value.backgroundFocalX ?? 50, 0, 100, "Background focal X"), backgroundFocalY: finiteInteger(value.backgroundFocalY ?? 50, 0, 100, "Background focal Y"), backgroundImageOpacity: finiteInteger(value.backgroundImageOpacity ?? 72, 0, 100, "Background opacity"), backgroundOverlayIntensity: finiteInteger(value.backgroundOverlayIntensity ?? 58, 0, 100, "Background overlay"), winnerMessageTemplate: boundedText(value.winnerMessageTemplate || THIRD_RAIL_GOLD_CONFIG.winnerMessageTemplate, 1, WHEEL_FILE_LIMITS.winnerMessage, "Winner message"), publicHistoryVisible: value.publicHistoryVisible !== false };
}
...
function normalizePortableStyle(value, fallback) { if (!isRecord(value)) throw new Error("A portable segment style is invalid."); const mode = String(value.mode || "solid"); const color = normalizeHex(value.color || fallback, "Segment colour"); if (mode === "solid") { rejectUnknown(value, new Set(["mode", "color"]), "segment style"); return { mode, color }; } if (mode === "pattern") { rejectUnknown(value, new Set(["mode", "color", "pattern", "patternColor"]), "segment style"); const style = normalizeSegmentStyle(value, color); return style; } if (mode === "image") { rejectUnknown(value, new Set(["mode", "color", "imageAssetRef"]), "segment style"); const imageAssetRef = String(value.imageAssetRef || ""); if (!/^segment-[a-f0-9]{12,64}$/.test(imageAssetRef)) throw new Error("An image style has an invalid logical segment reference."); return { mode, color, imageAssetRef }; } throw new Error("A portable segment fill mode is invalid."); }
...
function runtimeStyle(value, refs) { if (value.mode !== "image") return value; const imageAssetId = refs.get(value.imageAssetRef); if (!imageAssetId) throw new Error("A segment image style references missing embedded media."); return { mode: "image", color: value.color, imageAssetId }; }
