...
async function parseCanonical(document, sourceName, defaults) {
  ...
  if (document.formatVersion !== WHEEL_FILE_FORMAT_VERSION) throw new Error(`Third Railify wheel format version ${document.formatVersion} is not supported.`);
  ...
  for (const [purpose, item] of Object.entries(wheel.media)) if (item) { const actualMediaHash = await sha256Hex(decodeBase64(item.base64)); if (actualMediaHash !== item.sha256) throw new Error(`The embedded ${purpose} image hash does not match.`); }
  ...
  if (document.integrity != null) {
    ...
    const actual = await sha256Hex(canonicalStringify(wheel));
  ...
  const proposal = { title: wheel.title, description: wheel.description, config: wheel.settings, entries: wheel.entries.map((entry) => ({ id: newId(), label: entry.label, order: entry.order, weight: entry.weight, colour: entry.color, state: entry.active ? "active" : "hidden" })), media: wheel.media, messages: [info("integrity", integrityStatus === "verified" ? "verified" : "absent", integrityStatus === "verified" ? "SHA-256 matched the canonical wheel payload. This detects corruption; it is not a signature." : "This supported file has no integrity hash."), info("entry identity", "fresh local IDs", "Portable entries never control authoritative entry identity."), info("official history and access", "never imported", "Portable files contain creator-editable content only.")], sourceIndex: 0 };
  return { detectedFormat: "thirdrailify", formatLabel: "Third Railify wheel", version: WHEEL_FILE_FORMAT_VERSION, sourceName, proposals: [finalizeProposal(proposal, integrityStatus)] };
}
...
function normalizePortableWheel(value, defaults = THIRD_RAIL_GOLD_CONFIG) {
  ...
  const settings = normalizeSettings(value.settings || defaults);
  ...
  const entries = value.entries.map((entry, index) => normalizePortableEntry(entry, index)).sort((a, b) => a.order - b.order || a._sourceOrder - b._sourceOrder).map((entry, order) => ({ label: entry.label, weight: entry.weight, color: entry.color, active: entry.active, order }));
  const mediaValue = value.media == null ? {} : value.media;
  if (!isRecord(mediaValue)) throw new Error("Portable media metadata is invalid.");
  rejectUnknown(mediaValue, new Set(["background", "center"]), "wheel.media");
  const media = { background: normalizeEmbeddedMedia(mediaValue.background, "background"), center: normalizeEmbeddedMedia(mediaValue.center, "center") };
  const totalMedia = [media.background, media.center].reduce((total, item) => total + (item ? decodeBase64(item.base64).byteLength : 0), 0);
  if (totalMedia > WHEEL_FILE_LIMITS.embeddedMediaBytes) throw new Error("Embedded wheel media exceeds the 12 MB total limit.");
  return { title, description, settings, entries, media };
}

function normalizePortableEntry(entry, index) {
  if (!isRecord(entry)) throw new Error(`Entry ${index + 1} is invalid.`);
  rejectUnknown(entry, new Set(["label", "weight", "color", "active", "order"]), `entry ${index + 1}`);
  ...
  return { label: boundedText(entry.label, 1, WHEEL_FILE_LIMITS.entryLabel, `Entry ${index + 1} label`), weight, color, active: entry.active !== false, order, _sourceOrder: index };
}

function normalizeSettings(value) {
  if (!isRecord(value)) throw new Error("Wheel settings are invalid.");
  rejectUnknown(value, new Set(SETTINGS), "wheel.settings");
  const palette = Array.isArray(value.palette) ? value.palette.map((colour, index) => normalizeHex(colour, `Palette colour ${index + 1}`)) : [...THIRD_RAIL_GOLD_CONFIG.palette];
  const preset = new Set(["third-rail-gold", "live-wire-red", "gina-violet", "high-voltage-mono", "signal-teal", "after-hours", "custom"]).has(value.themePreset) ? value.themePreset : "third-rail-gold";
  const minimum = preset === "custom" ? 1 : 2; const maximum = preset === "custom" ? 5 : WHEEL_FILE_LIMITS.palette;
  if (palette.length < minimum || palette.length > maximum) throw new Error(preset === "custom" ? "Custom palettes must contain between 1 and 5 colours." : "Wheel palettes must contain between 2 and 12 colours.");
  return { themePreset: preset, palette, pointerAccent: normalizeHex(value.pointerAccent || THIRD_RAIL_GOLD_CONFIG.pointerAccent, "Pointer accent"), centreTreatment: oneOf(value.centreTreatment, ["bolt", "signal", "ring"], "bolt"), backgroundIntensity: oneOf(value.backgroundIntensity, ["low", "medium", "high"], "high"), labelContrast: oneOf(value.labelContrast, ["light", "dark"], "light"), spinDurationMs: finiteInteger(value.spinDurationMs ?? 6500, 2000, 20000, "Spin duration"), tickingSoundEnabled: value.tickingSoundEnabled !== false, winnerSoundEnabled: value.winnerSoundEnabled !== false, celebrationEnabled: value.celebrationEnabled !== false, confettiEnabled: value.confettiEnabled !== false, fireworksEnabled: value.fireworksEnabled !== false, winnerLightingEnabled: value.winnerLightingEnabled !== false, celebrationIntensity: oneOf(value.celebrationIntensity, ["subtle", "normal", "strong"], "normal"), backgroundEnabled: value.backgroundEnabled !== false, backgroundFocalX: finiteInteger(value.backgroundFocalX ?? 50, 0, 100, "Background focal X"), backgroundFocalY: finiteInteger(value.backgroundFocalY ?? 50, 0, 100, "Background focal Y"), backgroundImageOpacity: finiteInteger(value.backgroundImageOpacity ?? 72, 0, 100, "Background opacity"), backgroundOverlayIntensity: finiteInteger(value.backgroundOverlayIntensity ?? 58, 0, 100, "Background overlay"), winnerMessageTemplate: boundedText(value.winnerMessageTemplate || THIRD_RAIL_GOLD_CONFIG.winnerMessageTemplate, 1, WHEEL_FILE_LIMITS.winnerMessage, "Winner message"), publicHistoryVisible: value.publicHistoryVisible !== false };
}
...
function convertGeneric(document, defaults, sourceName) {
  ...
  return { title, description: isRecord(document) ? boundedText(document.description || "", 0, WHEEL_FILE_LIMITS.description, "Description") : "", config: { ...defaults, palette: [...defaults.palette] }, entries, media: { background: null, center: null }, messages, sourceIndex: 0 };
}
...
export async function embedCurrentWheelMedia(wheel, fetchImpl = globalThis.fetch) {
  const result = { background: null, center: null };
  ...
  const total = Object.values(result).reduce((sum, item) => sum + (item ? decodeBase64(item.base64).byteLength : 0), 0);
...
function normalizeEmbeddedMedia(value, purpose) {
  ...
  rejectUnknown(value, new Set(["mode", "fileName", "mimeType", "sha256", "base64"]), `${purpose} media`);
...
function validateMedia(bytes, mimeType, purpose) { const limit = purpose === "background" ? WHEEL_FILE_LIMITS.backgroundBytes : WHEEL_FILE_LIMITS.centerBytes; if (!bytes.byteLength || bytes.byteLength > limit) throw new Error(`Embedded ${purpose} media must be between 1 byte and ${limit / 1024 / 1024} MB.`); const detected = detectMediaType(bytes); if (!detected || detected !== mimeType) throw new Error(`Embedded ${purpose} media is not a valid ${mimeType} image.`); if (mimeType === "image/svg+xml") validateSvg(bytes); }
function detectMediaType(bytes) { if (bytes.length >= 3 && bytes[0] === 0xff && bytes[1] === 0xd8 && bytes[2] === 0xff) return "image/jpeg"; if (bytes.length >= 8 && [0x89,0x50,0x4e,0x47,0x0d,0x0a,0x1a,0x0a].every((value, index) => bytes[index] === value)) return "image/png"; if (bytes.length >= 12 && ascii(bytes, 0, 4) === "RIFF" && ascii(bytes, 8, 12) === "WEBP") return "image/webp"; if (bytes.length >= 2 && ascii(bytes, 0, 2) === "BM") return "image/bmp"; try { if (/^<svg[\s>]/i.test(new globalThis.TextDecoder("utf-8", { fatal: true }).decode(bytes.slice(0, 512)).replace(/^\uFEFF/, "").trimStart())) return "image/svg+xml"; } catch { return ""; } return ""; }
