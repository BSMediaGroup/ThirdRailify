    ...
    <div ref={root} className="wheel-modal wheel-transfer-dialog" role="dialog" aria-modal="true" aria-labelledby="wheel-transfer-title">
      ...
      <div className="wheel-transfer-body">
        {tab === "import" || !canExport ? <section className="wheel-transfer-import" aria-label="Import wheel content">
          ...
          {proposal && confirming ? <div className="wheel-replace-confirm" role="alert"><strong>Load imported content into this editor?</strong><dl><div><dt>Title</dt><dd>{source.title} → {proposal.title}</dd></div><div><dt>Participants</dt><dd>{source.entries.length} → {proposal.entries.length}</dd></div><div><dt>Custom media</dt><dd>{mediaChange(currentMedia || source.media, proposal.media)}</dd></div></dl><p>Wheel identity, owner, grants, locks and official result history remain unchanged. Nothing is persisted until you press the editor’s existing Save action.</p><div><button type="button" onClick={() => setConfirming(false)}>Back to review</button><button type="button" className="primary-button" onClick={apply}>Load import into editor</button></div></div> : null}
          ...
          <div className="wheel-export-actions"><button type="button" className="primary-button" disabled={busy} onClick={() => void exportWheel("twl")}>Download .twl</button><button type="button" className="secondary-button" disabled={busy} onClick={() => void exportWheel("json")}>Download JSON</button><button type="button" disabled={busy} onClick={() => void exportWheel("copy")}><CopyIcon /> Copy JSON</button></div>
      ...
      <footer className="wheel-modal__footer"><button type="button" onClick={requestClose} disabled={busy}>Return to editor</button>{tab === "import" && proposal && !confirming ? <button type="button" className="primary-button" disabled={busy} onClick={apply}>{mode === "new" ? "Use as new wheel" : "Review replacement"}</button> : null}</footer>
    </div>
  </div>, document.body);
}
