...
export function CartPage() {
  ...
  return <section className="cart-page"><div className="container">
    ...
    {rows.length ? <div className="cart-page__layout">
      ...
      <aside className="cart-summary" aria-labelledby="cart-summary-title"><p className="eyebrow">Cart summary</p><h2 id="cart-summary-title">Subtotal</h2><CadAmount minorUnits={subtotal} className="cart-summary__amount" /><ProductCurrencyComparison cadPrice={subtotal / 100} /><p>Prices shown here come from Commerce D1 and are re-resolved by the server. Shipping is requested from the delivery step; checkout remains closed.</p><Link className="button button--primary" to="/checkout">Enter delivery details</Link><Link className="button button--secondary" to="/shop">Continue shopping</Link><button className="text-button" type="button" onClick={cart.clear}>Clear cart</button></aside>
