...
export function CartPage() {
  ...
  useEffect(() => {
    ...
    catalogueProvider.load(controller.signal)
      .then((snapshot) => { setProducts(snapshot.products); setCheckoutEnabled(snapshot.checkoutEnabled === true); setCatalogueReady(true); setError(""); })
  ...
  return <section className="cart-page"><div className="container">
    ...
    {cart.items.length ? <div className="cart-page__layout">
      ...
      <aside className="cart-summary" aria-labelledby="cart-summary-title"><p className="eyebrow">Order summary</p><h2 id="cart-summary-title">Subtotal</h2><CadAmount minorUnits={subtotal} className="cart-summary__amount" /><ProductCurrencyComparison cadPrice={subtotal / 100} />{unavailable.length ? <div className="checkout-error" role="alert"><strong>{unavailable.length} unavailable cart item{unavailable.length === 1 ? "" : "s"}</strong><p>Remove stale items before checkout. They are excluded from the subtotal.</p></div> : null}<dl className="cart-summary__costs"><div><dt>Shipping</dt><dd>Calculated at checkout</dd></div><div><dt>Taxes</dt><dd>Calculated before payment</dd></div></dl>{account && <section className="cart-delivery-summary"><span>Delivery</span>{commerce.loading ? <p>Loading your default address…</p> : defaultAddress ? <><strong>{defaultAddress.label}</strong><p>{defaultAddress.city}{defaultAddress.region ? `, ${defaultAddress.region}` : ""} · {defaultAddress.countryCode}</p><Link to="/account/delivery">Change address</Link></> : <><strong>No saved address</strong><Link to="/account/delivery">Add delivery address</Link></>}</section>}<p>{checkoutEnabled && !unavailable.length ? "The server will confirm delivery, tax, and payment prerequisites before handoff." : "Checkout is currently unavailable. No order or payment can be created."}</p>{checkoutEnabled && !unavailable.length ? <Link className="button button--primary" to="/checkout">Continue to checkout</Link> : <button className="button button--primary" type="button" disabled>{unavailable.length ? "Remove unavailable items" : "Checkout unavailable"}</button>}<Link className="button button--secondary" to="/checkout">Review delivery readiness</Link><Link className="button button--secondary" to="/shop">Continue shopping</Link><button className="text-button" type="button" onClick={cart.clear}>Clear cart</button></aside>
