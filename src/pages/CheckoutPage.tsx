...
import { CityLocalityField } from "../components/CityLocalityField";
import { CountryRegionFields } from "../components/CountryRegionFields";
import { formatGeography, normalizeCountry, normalizeRegion, postalLabel } from "../address/geography";
  ...
  useEffect(() => {
    ...
    fetch("/api/commerce/shipping-markets", { headers: { Accept: "application/json" }, signal: controller.signal })
      ...
      .then(({ response, payload }) => {
        ...
        setDelivery((current) => payload.markets!.some((market) => market.countryCode === current.countryCode) ? current : { ...current, countryCode: payload.markets![0].countryCode, region: "" });
      })
      ...
      <form className="checkout-form" onSubmit={requestRates} noValidate>
        ...
        <section className="checkout-panel" aria-labelledby="delivery-title"><p className="eyebrow">02 · Delivery details</p><h2 id="delivery-title">Where should it go?</h2><p className="checkout-panel__note">Worldwide shipping. Available methods and rates are confirmed for your destination and items.</p>
          ...
          <div className="checkout-fields">
            ...
            <CityLocalityField idPrefix="checkout-address" value={delivery.city} countryCode={delivery.countryCode} region={delivery.region} onChange={(next) => change("city", next)} error={touched ? errors.city : undefined} />
            <CountryRegionFields idPrefix="checkout-address" className="checkout-geography-fields" countryCode={delivery.countryCode} region={delivery.region} onCountryChange={(next) => change("countryCode", next)} onRegionChange={(next) => change("region", next)} allowedCountryCodes={shippingMarkets.map((market) => market.countryCode)} countryDisabled={!shippingMarkets.length} countryError={touched ? errors.countryCode : undefined} regionError={touched ? errors.region : undefined} countryHint="Only destinations enabled by commerce operations are shown." />
            <CheckoutField label={postalLabel(delivery.countryCode)} name="postalCode" value={delivery.postalCode} change={change} error={touched ? errors.postalCode : undefined} autoComplete="postal-code" />
...
function validateDelivery(value: Delivery) { const errors: Partial<Record<keyof Delivery, string>> = {}; if (!value.name.trim()) errors.name = "Enter the recipient name."; if (!value.address1.trim()) errors.address1 = "Enter the delivery street address."; if (!value.city.trim()) errors.city = "Enter the city or locality."; if (REGION_REQUIRED.has(value.countryCode.trim().toUpperCase()) && !value.postalCode.trim()) errors.postalCode = "Enter the postal or ZIP code."; const country = value.countryCode.trim().toUpperCase(); if (!/^[A-Z]{2}$/.test(country)) errors.countryCode = "Enter a two-letter country code."; if (REGION_REQUIRED.has(country) && !value.region.trim()) errors.region = "State, province, or region is required for this country."; return errors; }
...
function deliveryLabel(value: Rate["delivery"]) { if (!value) return "Delivery estimate not supplied"; if (value.text) return value.text; if (value.minDays && value.maxDays) return `${value.minDays}–${value.maxDays} business days`; if (value.maxDays) return `Up to ${value.maxDays} business days`; if (value.minDate && value.maxDate) return `${value.minDate}–${value.maxDate}`; return "Delivery estimate not supplied"; }
function deliveryFromAddress(address: AccountAddress): Delivery { const countryCode = normalizeCountry(address.countryCode); return { name: address.recipientName, company: address.company || "", address1: address.address1, address2: address.address2 || "", city: address.city, region: normalizeRegion(countryCode, address.region || ""), postalCode: address.postalCode, countryCode, phone: address.phone || "" }; }
