...
export function EpisodeDetailPage() {
  ...
  return (
    <article className="episode-detail">
      <header className="container episode-detail__header"><nav aria-label="Breadcrumb"><Link to="/watch">Watch</Link><span>/</span><Link to="/watch/episodes">Episodes</Link><span>/</span><span>Archive {String(archive.position).padStart(2, "0")}</span></nav><p className="eyebrow">Retained transmission</p><h1>{item.title}</h1></header>
      <div className="container episode-detail__stage"><BroadcastPlayer candidate={item} eager /></div>
      ...
      <nav className="container episode-neighbours" aria-label="Visible episode navigation"><span>{archive.previous ? <Link to={archive.previous.route}>← {archive.previous.title}</Link> : "Newest visible episode"}</span><Link to="/watch">Back to Watch</Link><span>{archive.next ? <Link to={archive.next.route}>{archive.next.title} →</Link> : "Oldest visible episode"}</span></nav>
    </article>
  );
}
