  ...
  return <main className="goat-detail">
    ...
    <section className="container goat-detail__hero">
      ...
      <div className="goat-detail__copy">
        ...
        <div className={`goat-detail__identity${item.media.profile ? " has-profile" : ""}${item.displayName.trim().length > 12 ? " is-long" : ""}`}>{item.media.profile ? <img src={item.media.profile.url} alt="" width="78" height="78" /> : null}<div><h1>{item.displayName}</h1><p className="goats-location-tag"><CountryFlag countryCode={item.location.countryCode} />{item.location.label}</p></div></div>
