...
function WheelCard({ wheel }: { wheel: WheelSummary }) { return <article className="wheel-card wheel-card--single" style={{ "--wheel-a": wheel.palette?.[0] || "#f3c928", "--wheel-b": wheel.palette?.[1] || "#b8182f" } as React.CSSProperties}><Link className="wheel-card__link" to={`/wheels/${wheel.slug}`} aria-label={`Open Wheel ${wheel.title}`} /><div className="wheel-card__art" aria-hidden="true"><span className="wheel-card__art-label">SINGLE DRAW</span><div className="wheel-card__orbit"><i /><i /><i /><WheelsBrandMark /></div></div><div className="wheel-card__copy"><p className="eyebrow">WHEEL</p><h3>{wheel.title}</h3><p>{wheel.description || "A public Third Railify competition Wheel."}</p><div className="wheel-card__meta"><span>{wheel.participantCount} participants</span><span>{wheel.weighted ? "Weighted" : "Equal weight"}</span><span>{wheel.officialEnabled ? "Official ready" : wheel.demoEnabled ? "Public demo" : "View only"}</span></div><span className="wheel-card__open">Open Wheel <span aria-hidden="true">↗</span></span></div><GalleryOwnerInfo owner={wheel.owner} itemTitle={wheel.title} itemType="Wheel" /></article>; }

function StageCard({ stage }: { stage: StageSummary }) {
  const palettes = stageCardPalettes(stage);
  return <article className="wheel-card wheel-card--stage" style={{ "--wheel-a": palettes[0][0], "--wheel-b": palettes[0][1] } as React.CSSProperties}><Link className="wheel-card__link" to={`/wheels/stages/${stage.slug}`} aria-label={`Open Stage ${stage.title}`} /><div className="wheel-card__art" aria-hidden="true"><span className="wheel-card__art-label">BROADCAST GROUP</span><div className="stage-card__emblem">{palettes.map((palette, index) => <i key={index} style={{ "--stage-card-a": palette[0], "--stage-card-b": palette[1] } as React.CSSProperties}><span>{index === 1 ? <WheelsBrandMark /> : null}</span></i>)}</div></div><div className="wheel-card__copy"><p className="eyebrow">STAGE</p><h3>{stage.title}</h3><p>{stage.description || "A public multi-wheel Third Railify Stage."}</p><div className="wheel-card__meta"><span>{stage.wheelCount} {stage.wheelCount === 1 ? "Wheel" : "Wheels"}</span><span>{participantTotal({ type: "stage", item: stage })} participants</span><span>Public</span></div><span className="wheel-card__open">Open Stage <span aria-hidden="true">↗</span></span></div><GalleryOwnerInfo owner={stage.owner} itemTitle={stage.title} itemType="Stage" /></article>;
}
