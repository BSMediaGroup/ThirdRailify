...
import { createGoatDraft, finaliseGoatDraft, getGoatConfig, getGoatProducts, searchGoatLocations, uploadGoatMedia } from "../goats/client";
import type { GoatConfig, GoatLocationSuggestion, GoatProduct } from "../goats/types";
...
export function GoatSubmitPage() {
  ...
  const chooseLocation = (location: GoatLocationSuggestion) => setFields((current) => ({ ...current, city: location.city, region: location.region, countryCode: location.countryCode }));
  ...
  return <main className="goat-submit"><section className="goat-submit__hero"><div className="container"><p className="eyebrow">Join the approved community showcase</p><h1>Submit your <span>GOATED drip.</span></h1><p>Four focused steps. Private photos until approval. Approximate locations only.</p></div></section>
    <div className="container goat-submit__layout"><aside><ol className="goat-stepper" aria-label="Submission progress">{steps.map((label, index) => <li key={label} className={index === step ? "is-current" : index < step ? "is-complete" : ""} aria-current={index === step ? "step" : undefined}><span>{String(index + 1).padStart(2, "0")}</span><strong>{label}</strong></li>)}</ol><div className="goat-privacy-note"><strong>Location privacy</strong><p>Search naturally—even with a full address if needed. The form discards street and house details and stores only a deliberately coarse city-level point.</p></div></aside>
      <form className="goat-submit__form" onSubmit={(event) => void submit(event)} noValidate aria-busy={busy}><header><p>Step {step + 1} of 4</p><h2>{steps[step]}</h2></header>{error ? <div className="goats-error" role="alert">{error}</div> : null}{progress && step !== 3 ? <p className="goat-submit__progress" role="status" aria-live="polite">{progress}</p> : null}{busy ? <SubmissionActivity progress={progress} /> : null}
        ...
        {step === 1 ? <fieldset><legend>Approximate location and real product</legend><label>Country<select required value={fields.countryCode} onChange={(event) => setFields((current) => ({ ...current, countryCode: event.target.value, region: "" }))}><option value="">Choose a country</option>{countries.map((country) => <option key={country.code} value={country.code}>{country.name}</option>)}</select></label><LocationLookup ref={firstField} fields={fields} onCityChange={(value) => update("city", value)} onChoose={chooseLocation} enabled={Boolean(config?.geocoderConfigured)} /><RegionField countryCode={fields.countryCode} value={fields.region} onChange={(value) => update("region", value)} /><label>Owned product<select required value={fields.productId} onChange={(event) => update("productId", event.target.value)}><option value="">Choose from the current catalogue</option>{products.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label>{product ? <article className="goat-product-preview" aria-label={`Selected product: ${product.name}`}><div className="goat-product-preview__image">{product.image ? <img src={product.image} alt="" width="120" height="120" /> : <span aria-hidden="true">TR</span>}</div><div><small>Selected catalogue product</small><strong>{product.name}</strong><span>Server-validated product · {product.slug}</span></div></article> : null}<p className="goat-field-note">A submission does not prove purchase. Product identity links the story to the real catalogue only.</p></fieldset> : null}
        {step === 2 ? <fieldset><legend>Your story and optional rating</legend><label>Story / review<textarea ref={firstField as unknown as React.RefObject<HTMLTextAreaElement>} required minLength={20} maxLength={2000} rows={9} value={fields.description} onChange={(event) => update("description", event.target.value)} /><small>{fields.description.length} / 2000 · Minimum 20 characters.</small></label><fieldset className="goat-stars"><legend>Product rating <span>Optional</span></legend><div role="radiogroup" aria-label="Optional product rating">{[1,2,3,4,5].map((rating) => <label key={rating} className={Number(fields.rating) >= rating ? "is-filled" : ""}><input type="radio" name="rating" value={rating} checked={fields.rating === String(rating)} onChange={(event) => update("rating", event.target.value)} /><span aria-hidden="true">★</span><small>{rating} star{rating === 1 ? "" : "s"}</small></label>)}<button type="button" onClick={() => update("rating", "")}>Clear rating</button></div></fieldset></fieldset> : null}
...
const LocationLookup = forwardRef<HTMLInputElement, { fields: DraftFields; enabled: boolean; onCityChange: (value: string) => void; onChoose: (value: GoatLocationSuggestion) => void }>(function LocationLookup({ fields, enabled, onCityChange, onChoose }, ref) {
  const [results, setResults] = useState<GoatLocationSuggestion[]>([]);
  const [loading, setLoading] = useState(false);
  const [open, setOpen] = useState(false);
  const selectedCity = useRef("");
  useEffect(() => {
    if (!enabled || fields.city.trim().length < 2 || fields.city === selectedCity.current) { setResults([]); setLoading(false); setOpen(false); return; }
    const controller = new AbortController();
    const timer = window.setTimeout(() => {
      setLoading(true);
      searchGoatLocations(fields.city.trim(), fields.countryCode, controller.signal).then((next) => { setResults(next); setOpen(true); }).catch((reason) => { if (reason?.name !== "AbortError") setResults([]); }).finally(() => setLoading(false));
    }, 320);
    return () => { window.clearTimeout(timer); controller.abort(); };
  }, [enabled, fields.city, fields.countryCode]);
  return <div className="goat-location-combobox"><label htmlFor="goat-location">City or location<input ref={ref} id="goat-location" required maxLength={140} autoComplete="off" value={fields.city} role="combobox" aria-autocomplete="list" aria-expanded={open && results.length > 0} aria-controls="goat-location-results" onFocus={() => setOpen(true)} onBlur={() => window.setTimeout(() => setOpen(false), 120)} onChange={(event) => { selectedCity.current = ""; onCityChange(event.target.value); setOpen(true); }} placeholder="Start typing a city, suburb, landmark, or address" /></label><small>{loading ? "Finding city-level matches…" : "Choose a match to fill the city, state/region, and country. Custom city text is also accepted."}</small>{open && results.length ? <ul id="goat-location-results" role="listbox">{results.map((result) => <li key={result.id} role="option" aria-selected={false}><button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { selectedCity.current = result.city; onChoose(result); setOpen(false); }}><strong>{result.city}</strong><span>{[result.region, result.countryName].filter(Boolean).join(", ")}</span><small>Use coarse city location</small></button></li>)}</ul> : null}</div>;
});

function RegionField({ countryCode, value, onChange }: { countryCode: string; value: string; onChange: (value: string) => void }) {
  const regions = REGIONS[countryCode] || [];
  if (!regions.length) return <label>State / region <span>Optional</span><input maxLength={100} value={value} onChange={(event) => onChange(event.target.value)} placeholder="Filled automatically when a location is chosen" /></label>;
  return <label>State / region <span>Optional</span><select value={value} onChange={(event) => onChange(event.target.value)}><option value="">Choose a state or region</option>{value && !regions.includes(value) ? <option value={value}>{value}</option> : null}{regions.map((region) => <option key={region} value={region}>{region}</option>)}</select></label>;
}

function SubmissionActivity({ progress }: { progress: string }) {
  return <div className="goat-submit__activity" role="status" aria-live="polite"><span className="goat-submit__activity-mark" aria-hidden="true"><i /><i /><i /></span><span><strong>Submission in progress</strong><small>{progress || "Preparing your private moderation record…"}</small></span></div>;
}
