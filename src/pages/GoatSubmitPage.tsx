...
export function GoatSubmitPage() {
  ...
  const chooseFile = async (file: File | undefined, setter: (file: File | null) => void) => {
    if (!file) return; setError("");
    try { setProgress("Preparing a metadata-free display image…"); setter(await prepareImage(file, config?.limits.maxImageBytes || 10 * 1024 * 1024)); }
...
function ImageField({ label, required, file, onChoose, onRemove }: { label: string; required?: boolean; file: File | null; onChoose: (file?: File) => void | Promise<void>; onRemove: () => void }) {
  const [preview, setPreview] = useState("");
  useEffect(() => { if (!file) { setPreview(""); return; } const url = URL.createObjectURL(file); setPreview(url); return () => URL.revokeObjectURL(url); }, [file]);
  return <label className="goat-upload">{label} {required ? <span aria-hidden="true">*</span> : <span>Optional</span>}<input type="file" required={required} accept="image/jpeg,image/png,image/webp" onChange={(event) => onChoose(event.target.files?.[0])} />{file ? <span className="goat-upload__preview"><img src={preview} alt="Selected preview" width="120" height="120" /><span>{file.name}<small>{formatBytes(file.size)}</small></span><button type="button" onClick={onRemove}>Remove</button></span> : <small>Choose a JPG, PNG, or WebP image up to 10 MB.</small>}</label>;
}
...
async function prepareImage(file: File, maxBytes: number) {
  if (!new Set(["image/jpeg", "image/png", "image/webp"]).has(file.type) || file.size > maxBytes) throw new Error("Images must be valid JPG, PNG, or WebP files no larger than 10 MB.");
