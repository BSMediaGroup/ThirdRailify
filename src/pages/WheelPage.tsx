...
export function WheelPage({ presentation = false, editorRequested = false }: { presentation?: boolean; editorRequested?: boolean }) {
  ...
  const [activePayload, setActivePayload] = useState<LoadedWheel | null>(null); const activeRef = useRef<LoadedWheel | null>(null);
  ...
  useEffect(() => {
    ...
    getWheel(slug).then((payload) => {
      ...
      if (current && current.wheel.slug !== incoming.wheel.slug && !presentation) {
      ...
      if (current && current.wheel.slug !== slug) {
        ...
        navigate(`/wheels/${current.wheel.slug}`, { replace: true, state: { wheelSceneNavigation: true } });
  ...
  const finishSpin = useCallback(() => { setSpinning(false); audio.stopTicks(); const next = pending.current; pending.current = null; if (!next) return; setResult(next); if (activeRef.current?.wheel.config.winnerSoundEnabled && !soundMuted) audio.playWinner(); }, [audio, soundMuted]);
  const spin = async () => {
    ...
    try {
      ...
      if (mode === "official") {
        ...
        entry = active.find((candidate) => candidate.id === response.spin.winningEntryId) || { id: response.spin.winningEntryId, label: response.spin.winningLabel, order: 0, weight: 1, colour: null, state: "active" }; official = true;
      ...
      if (!soundMuted && activeWheel.config.tickingSoundEnabled && !reduced) audio.startTicks(plan.durationMs); if (reduced) window.requestAnimationFrame(() => finishSpin());
  ...
  const navigateToWheel = (summary: WheelSummary | null, direction: WheelDirection) => { if (!summary || navigationLocked) return; sceneScroll.current = window.scrollY; navigationFocus.current = direction; setParticipantDetail(null); setNotice(""); navigate(`/wheels/${summary.slug}`, { state: { wheelSceneNavigation: true, wheelNavigationDirection: direction } }); };
  const sceneProps: ActiveSceneProps = { presentation, mode, rotation, spinning, spinRequestPending, soundMuted, result, error, neighbours, navigationLocked, account: Boolean(account), canOpenAppearance: Boolean(access.canEdit && csrfToken), onNavigate: navigateToWheel, onSpin: spin, onFinishSpin: finishSpin, onMode: setMode, onToggleSound: toggleSound, onToggleFullscreen: toggleFullscreen, onPointerTarget: setPointerTarget, onParticipant: (entry, trigger) => setParticipantDetail({ entry, trigger }), onOpenParticipants: openParticipantManager, onOpenAppearance: () => setAppearanceOpen(true), onLogin: () => openAuth("signin"), pointerTarget };
  ...
  return <div className={`wheel-control-page${presentation ? " wheel-control-page--presentation" : ""} intensity--${wheel.config.backgroundIntensity}${wheelBackground(wheel) ? " has-custom-background" : ""}${transition ? " is-scene-transitioning" : ""}`} style={wheelSurfaceStyle(wheel)} data-wheel-transition={transition ? transition.direction : "settled"}>
    ...
    {appearanceOpen && access.canEdit && csrfToken ? <AppearanceDialog wheel={{ ...wheel, media: wheel.media || { background: null, centre: null } }} draft={appearanceDraft} csrfToken={csrfToken} onClose={() => { setAppearanceOpen(false); setAppearanceDraft(undefined); }} onSaved={applyWheel} /> : null}
  </div>;
}

type ActiveSceneProps = { presentation: boolean; mode: "practice" | "official"; rotation: number; spinning: boolean; spinRequestPending: boolean; soundMuted: boolean; result: { entry: WheelEntry; official: boolean } | null; error: string; neighbours: ReturnType<typeof wheelGalleryNeighbours>; navigationLocked: boolean; account: boolean; canOpenAppearance: boolean; pointerTarget: WheelEntry | null; onNavigate: (wheel: WheelSummary | null, direction: WheelDirection) => void; onSpin: () => Promise<void>; onFinishSpin: () => void; onMode: (mode: "practice" | "official") => void; onToggleSound: () => void; onToggleFullscreen: () => void; onPointerTarget: (entry: WheelEntry | null) => void; onParticipant: (entry: WheelEntry, trigger: HTMLElement) => void; onOpenParticipants: () => void; onOpenAppearance: () => void; onLogin: () => void };
...
function WheelScene({ payload, role, direction, interactive, ...props }: { payload: LoadedWheel; role: "active" | "incoming" | "outgoing"; direction: WheelDirection | null; interactive: boolean } & ActiveSceneProps) {
  const { wheel, access } = payload; const active = wheel.entries.filter((entry) => entry.state === "active"); const officialAvailable = Boolean(access.canSpinOfficially && wheel.officialEnabled && wheel.lifecycle === "active"); const locked = props.navigationLocked || !interactive;
