...
export function WheelPage({ presentation = false, editorRequested = false }: { presentation?: boolean; editorRequested?: boolean }) {
  ...
  const [mode, setMode] = useState<"practice" | "official">("practice"); const [rotation, setRotation] = useState(0); const [animation, setAnimation] = useState<WheelSpinPlan | null>(null); const [spinning, setSpinning] = useState(false); const [spinRequestPending, setSpinRequestPending] = useState(false); const [soundMuted, setSoundMuted] = useState(false);
  ...
  useEffect(() => {
    ...
    getWheel(slug).then((payload) => {
      ...
      if (current && current.wheel.slug !== incoming.wheel.slug) {
        ...
        pending.current = null; stopTicks(); setSpinning(false); setAnimation(null); setSpinRequestPending(false); setResult(null); setBusyAction(false); setMode("practice"); setRotation(0); setPointerTarget(null); setParticipantDetail(null); setAppearanceOpen(false); setAppearanceDraft(undefined);
  ...
  useWheelRefresh(slug, loading || spinning || spinRequestPending || Boolean(result) || busyAction || routePending || Boolean(transition) || editorRequested || searchParams.get('dialog') === 'participants' || appearanceOpen,
    () => getWheel(slug, { force: true }), next => {
      const current = activeRef.current; if (refreshHeldForPreflight.current || !current || current.wheel.slug !== next.wheel.slug || JSON.stringify(current) === JSON.stringify(next)) return;
      activeRef.current = next; setActivePayload(next);
    });

  const finishSpin = useCallback(() => { setSpinning(false); setAnimation(null); audio.stopTicks(); const next = pending.current; pending.current = null; if (!next) return; setResult(next); const config = activeRef.current?.wheel.config; if (config?.winnerSoundEnabled && !soundMuted) audio.playWinner(config.winnerSoundPreset || "gold-rise"); }, [audio, soundMuted]);
  const spin = async () => {
    ...
    try {
      let entry: WheelEntry; let official = false; let officialPlan: { landingFraction: number; turnRandom: number } | null = null; let planId: string = crypto.randomUUID();
      if (mode === "official") {
      ...
      const mechanics = await getWheelMechanics(); const plan = { ...spinPlan(active, entry.id, activeWheel.config.spinDurationMs, rotation, { ...(officialPlan || {}), mechanics: mechanics.mechanics, mechanicsRevision: mechanics.revision }), id: planId };
      pending.current = { entry, official }; setAnimation(plan); setSpinning(true); setRotation(plan.finalRotation);
    } catch (reason) { setError(message(reason)); setSpinning(false); setAnimation(null); pending.current = null; audio.stopTicks(); } finally { refreshHeldForPreflight.current = false; setSpinRequestPending(false); }
  };
  ...
  const sceneProps: ActiveSceneProps = { presentation, mode, rotation, animation, spinning, spinRequestPending, soundMuted, result, error, neighbours, navigationLocked, account: Boolean(account), canCreateStage: Boolean(canCreateStage && access.role === "owner"), canOpenAppearance: Boolean(access.canEdit && csrfToken), onNavigate: navigateToWheel, onShare: shareWheel, onSpin: spin, onFinishSpin: finishSpin, onBoundaryCrossing: (count, preset) => { if (!soundMuted) audio.playTicks(count, preset); }, onMode: setMode, onToggleSound: toggleSound, onToggleFullscreen: toggleFullscreen, onPointerTarget: setPointerTarget, onParticipant: (entry, trigger) => setParticipantDetail({ entry, trigger }), onOpenParticipants: openParticipantManager, onOpenAppearance: () => setAppearanceOpen(true), onLogin: () => openAuth("signin"), pointerTarget };
  ...
  return <div className={`wheel-control-page${presentation ? " wheel-control-page--presentation" : ""} intensity--${wheel.config.backgroundIntensity}${wheelBackground(wheel) ? " has-custom-background" : ""}${transition ? " is-scene-transitioning" : ""}`} style={wheelSurfaceStyle(wheel)} data-wheel-transition={transition ? transition.direction : "settled"}>
    ...
    <EphemeralNotices notice={notice} error={error} noticeTone="info" noticeTitle="Wheel notice" errorTitle="Wheel action unavailable" onDismissNotice={dismissNotice} onDismissError={() => setError("")} />
...
type ActiveSceneProps = { presentation: boolean; mode: "practice" | "official"; rotation: number; animation: WheelSpinPlan | null; spinning: boolean; spinRequestPending: boolean; soundMuted: boolean; result: { entry: WheelEntry; official: boolean } | null; error: string; neighbours: ReturnType<typeof wheelGalleryNeighbours>; navigationLocked: boolean; account: boolean; canCreateStage: boolean; canOpenAppearance: boolean; pointerTarget: WheelEntry | null; onNavigate: (wheel: WheelSummary | null, direction: WheelDirection) => void; onShare: () => Promise<void>; onSpin: () => Promise<void>; onFinishSpin: () => void; onBoundaryCrossing: (count: number, preset: string) => void; onMode: (mode: "practice" | "official") => void; onToggleSound: () => void; onToggleFullscreen: () => void; onPointerTarget: (entry: WheelEntry | null) => void; onParticipant: (entry: WheelEntry, trigger: HTMLElement) => void; onOpenParticipants: () => void; onOpenAppearance: () => void; onLogin: () => void };
...
function useWheelAudio() {
  const context = useRef<AudioContext | null>(null); const tickNodes = useRef(new Set<OscillatorNode>()); const stingerNodes = useRef<OscillatorNode[]>([]); const closeTimer = useRef<number | null>(null);
  const unlock = useCallback(() => { try { context.current ||= new AudioContext(); if (context.current.state === "suspended") void context.current.resume(); } catch { /* optional */ } }, []);
  const stopTicks = useCallback(() => { for (const node of tickNodes.current) try { node.stop(); } catch { /* already stopped */ } tickNodes.current.clear(); }, []);
  const playTicks = useCallback((count: number, preset: string) => { const audio = context.current; const profile = spinSoundProfile(preset); if (!audio || audio.state !== "running" || !profile) return; for (let index = 0; index < Math.min(3, Math.max(1, count)); index += 1) { const oscillator = audio.createOscillator(); const gain = audio.createGain(); const start = audio.currentTime + index * .006; oscillator.type = profile.waveform; oscillator.frequency.value = profile.frequency; oscillator.detune.value = profile.detune; gain.gain.setValueAtTime(.0001, start); gain.gain.exponentialRampToValueAtTime(profile.gain, start + profile.attack); gain.gain.exponentialRampToValueAtTime(.0001, start + profile.decay); oscillator.connect(gain).connect(audio.destination); tickNodes.current.add(oscillator); oscillator.start(start); oscillator.stop(start + profile.decay + .01); oscillator.addEventListener("ended", () => tickNodes.current.delete(oscillator)); } }, []);
  const stopWinner = useCallback(() => { for (const node of stingerNodes.current) try { node.stop(); } catch { /* stopped */ } stingerNodes.current = []; if (closeTimer.current != null) window.clearTimeout(closeTimer.current); closeTimer.current = null; }, []);
  const playWinner = useCallback((preset: string) => { stopWinner(); const audio = context.current; const profile = winnerSoundProfile(preset); if (!audio || audio.state !== "running" || !profile) return; profile.notes.forEach((frequency, index) => { const oscillator = audio.createOscillator(); const gain = audio.createGain(); oscillator.type = profile.waveform; oscillator.frequency.value = frequency; const start = audio.currentTime + index * profile.spacing; gain.gain.setValueAtTime(.0001, start); gain.gain.exponentialRampToValueAtTime(profile.gain, start + .018); gain.gain.exponentialRampToValueAtTime(.0001, start + profile.decay); oscillator.connect(gain).connect(audio.destination); oscillator.start(start); oscillator.stop(start + profile.decay + .03); stingerNodes.current.push(oscillator); }); closeTimer.current = window.setTimeout(stopWinner, Math.ceil((profile.notes.length * profile.spacing + profile.decay + .2) * 1000)); }, [stopWinner]);
  const cleanup = useCallback(() => { stopTicks(); stopWinner(); const audio = context.current; context.current = null; if (audio) void audio.close(); }, [stopTicks, stopWinner]);
  return useMemo(() => ({ unlock, playTicks, stopTicks, playWinner, cleanup }), [cleanup, playTicks, playWinner, stopTicks, unlock]);
}
