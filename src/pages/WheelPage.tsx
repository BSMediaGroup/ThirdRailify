...
export function WheelPage({ presentation = false, editorRequested = false }: { presentation?: boolean; editorRequested?: boolean }) {
  ...
  const sceneProps: ActiveSceneProps = { presentation, mode, rotation, animation, spinning, spinRequestPending, soundMuted, result, error, neighbours, navigationLocked, account: Boolean(account), canCreateStage: Boolean(canCreateStage && access.role === "owner"), canOpenAppearance: Boolean(access.canEdit && csrfToken), onNavigate: navigateToWheel, onShare: shareWheel, onSpin: spin, onFinishSpin: finishSpin, onMode: setMode, onToggleSound: toggleSound, onToggleFullscreen: toggleFullscreen, onPointerTarget: setPointerTarget, onParticipant: (entry, trigger) => setParticipantDetail({ entry, trigger }), onOpenParticipants: openParticipantManager, onOpenAppearance: () => setAppearanceOpen(true), onLogin: () => openAuth("signin"), pointerTarget };
