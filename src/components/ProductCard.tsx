...
export function ProductCard({ product, index = 0 }: { product: CatalogueProduct; index?: number }) {
  ...
  return (
    <article className="product-card product-card--commerce" style={{ "--card-index": index } as React.CSSProperties}>
      ...
      <ProductSaleStatus product={product} compact />
      <div className="product-card__body">
        ...
        <div className="product-card__footer">{product.saleRestriction?.enabled ? <span className="product-special-price">Collector showcase</span> : <ProductPrice price={product.price} formattedPrice={product.formattedPrice} />}<Link className="product-card__view" to={detailUrl}>View product <ArrowIcon /></Link></div>
      </div>
    </article>
  );
}
