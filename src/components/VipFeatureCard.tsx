...
export function VipFeatureCard({ compact = false }: { compact?: boolean }) {
  return <article className={`vip-feature-card${compact ? " vip-feature-card--compact" : ""}`}>
    ...
    <div className="vip-feature-card__content"><p className="eyebrow">THIRD RAILIFY / INNER RAIL</p><h2>The next member experience is being built properly.</h2><p>VIP V2 is still gated while account entitlements, billing, and purchase authority are rebuilt. Nothing on this staging route creates a membership or charge.</p></div>
