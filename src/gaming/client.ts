...
export type RotationResponse = { ok:true;schema:"thirdrailify-gaming-rotation-v1";items:Array<{id:string;title:string;platform:string;description:string;genre:string;artworkUrl:string|null;igdb?:{id:string;url:string}|null;steam:null|{appId:string;storeUrl:string};position:number}>;updatedAt:string|null };
export function useGamingRotation(){const [items,setItems]=useState<GamingRotationItem[]>([]);const [state,setState]=useState<"loading"|"ready"|"empty"|"unavailable">("loading");const load=useCallback(async()=>{try{const response=await fetch("/api/gaming/rotation",{headers:{Accept:"application/json"}});const payload=await response.json().catch(()=>null) as RotationResponse|null;if(!response.ok||!payload?.ok||!Array.isArray(payload.items))throw new Error("rotation_unavailable");const next=payload.items.sort((a,b)=>a.position-b.position).map(item=>({id:item.id,index:String(item.position).padStart(2,"0"),title:item.title,platform:item.platform,description:item.description,genre:item.genre,artworkUrl:item.artworkUrl,steam:item.steam,igdb:item.igdb,visual:visualFor(item.id,item.title)}));setItems(next);setState(next.length?"ready":"empty");}catch{setItems([]);setState("unavailable");}},[]);useEffect(()=>{void load();const interval=window.setInterval(()=>void load(),60_000);return()=>window.clearInterval(interval);},[load]);return{items,state,retry:load};}
