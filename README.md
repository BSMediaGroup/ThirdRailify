...
Managed Current Rotation responses additionally allow `igdb: { id, url } | null`. The relay allowlists those two persisted fields and discards invalid IGDB references. Cards display a compact IGDB LISTING / IGDB ID reference alongside a verified Steam listing when present, use the stored canonical URL with accessible external-link names, and omit absent provider rows entirely. Public code never queries `api.igdb.com` or `id.twitch.tv`; it consumes Third Railify's own persisted catalogue. Saved IGDB cover URLs can load as ordinary images through `img-src`, with existing fallback artwork. No Public credentials or lookup facility are added. Existing poster/square/mobile artwork ratios and the >= 9:16 geometry regression remain in the Gaming browser suite, including both-provider, single-provider and no-provider cases. No additional attribution statement was added.
...
Account Delivery and Checkout share `CountryRegionFields` plus the pinned offline `country-region-data@4.1.0` dataset (249 ISO alpha-2 countries and 4,387 region records). People choose names such as Canada and Ontario while explicit saves and provider-facing payloads retain canonical `CA` / `ON` values. Recognized legacy names and case variants hydrate canonically, unknown saved regions remain visible until explicitly changed, and summaries translate known codes without rewriting saved addresses or immutable order snapshots. The dataset adds no geocoding, location lookup, API key, runtime geography request, tracking, consent category, or schema migration.
...
## Product fullscreen gallery (local)
...
Aboot Nothing hero (2026-09-08): src/components/AbootNothingHero.tsx and src/styles/aboot-hero.css provide the animated comparison arena, shared gallery atmosphere, motion gating and responsive navigation to the directory. Both collection/alias SEO resolve to Aboot Nothing | Third Railify. Deployed 18c2899c; production browser verified 1440/390 widths, alias, title, animated/static motion and no overflow. Connected browser acceptance passed at four widths. Build/lint pass; SEO suite has one pre-existing /receipt description-length failure (5/6 pass). Read-only live evidence: .artifacts/aboot-hero.
