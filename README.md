...
Wheel entrant identity V2 keeps that Admin authority boundary while Public receives stable entrant codes, typed provenance, optional display suffixes, immutable result snapshots, and a bounded public activity projection. New shared contracts live in `src/lib/entrant-code.*` and `src/lib/entrant-label.*`; Wheel surfaces add `AutomationActivityDialog.tsx` and `CloseCreateNextDialog.tsx`. Canonical `.twl` format V4 round-trips suffix content and its display toggle while assigning fresh entrant IDs/codes on import. The Admin repository's `docs/WHEEL_ENTRANT_IDENTITY_V2.md` owns the schema and release order.
