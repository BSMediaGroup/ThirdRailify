...
test("Watch V2 routes, slot counts, players, precedence, redirect fallback, and responsive layouts", async (t) => {
  ...
  for (const [route, sizes] of Object.entries(MATRIX)) {
    for (const [width, height] of sizes) {
      ...
      if (route === "watch") {
        ...
        assert.equal(await watchHero.locator(".watch-trail-bg").count(), 1, "Watch hero uses the shared animated trail environment");
        assert.equal(await watchHero.locator(":scope > .signal-field,.watch-hero__signal,.watch-hero__atmosphere").count(), 0, "Watch hero removes all legacy background systems");
        assert.equal(await watchHero.locator(".watch-trail-bg__trail--ghost").count(), 4, "Watch hero carries four persistent signal trails");
        assert.equal(await watchHero.locator(".watch-trail-bg__trail--pulse").count(), 2, "Watch hero carries two animated pulse trails");
        assert.equal(await watchHero.locator(".watch-trail-bg__nodes circle").count(), 4, "Watch hero carries four signal nodes");
        assert.equal(await watchHero.locator(".watch-trail-bg__constellation i").count(), 9, "Watch hero carries the sparse signal constellation");
        assert.notEqual(await watchHero.locator(".watch-trail-bg__trail--pulse").first().evaluate((element) => getComputedStyle(element).animationName), "none", `Watch hero trails animate at ${width}x${height}`);
      ...
      if (route === "episodes") {
        ...
        assert.equal(await page.locator(".watch-trail-bg").count(), 1, "archive hero uses the shared animated trail environment");
        assert.equal(await page.locator(".episodes-signal-field").count(), 0, "the previous archive background system is absent");
        assert.equal(await page.locator(".watch-trail-bg__trail--ghost").count(), 4, "archive hero carries four persistent signal trails");
        assert.equal(await page.locator(".watch-trail-bg__trail--pulse").count(), 2, "archive hero carries two animated pulse trails");
        assert.equal(await page.locator(".watch-trail-bg__nodes circle").count(), 4, "archive hero carries four retained signal nodes");
        assert.equal(await page.locator(".watch-trail-bg__constellation i").count(), 9, "archive hero carries the sparse signal constellation");
        assert.notEqual(await page.locator(".watch-trail-bg__trail--pulse").first().evaluate((element) => getComputedStyle(element).animationName), "none", `archive trails animate at ${width}x${height}`);
  ...
  assert.equal(await reducedPage.locator(".watch-trail-bg__wash").evaluate((element) => getComputedStyle(element).animationName), "none", "archive hero light field respects reduced motion");
  assert.equal(await reducedPage.locator(".watch-trail-bg__trail--pulse").first().evaluate((element) => getComputedStyle(element).animationName), "none", "archive hero pulse trails respect reduced motion");
  assert.equal(await reducedPage.locator(".watch-trail-bg__beacon").evaluate((element) => getComputedStyle(element).animationName), "none", "archive hero depth beacon respects reduced motion");
  ...
  assert.equal(await reducedPage.locator(".watch-trail-bg__trail--pulse").first().evaluate((element) => getComputedStyle(element).animationName), "none", "Watch hero transmission trails respect reduced motion");
