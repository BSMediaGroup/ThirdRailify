...
async function assertPresentationViewport(page, label) { const result = await page.evaluate(() => { const surface = document.querySelector(".wheel-control-page--presentation")?.getBoundingClientRect(); const consoleBox = document.querySelector(".wheel-spin-console")?.getBoundingClientRect(); return { innerHeight: window.innerHeight, surface: surface?.toJSON(), console: consoleBox?.toJSON(), htmlOverflowY: getComputedStyle(document.documentElement).overflowY, bodyOverflowY: getComputedStyle(document.body).overflowY }; }); assert.ok(result.surface && result.surface.top >= -1 && result.surface.bottom <= result.innerHeight + 1, `${label} surface fits viewport: ${JSON.stringify(result)}`); assert.ok(result.console && result.console.bottom <= result.innerHeight + 1, `${label} console remains visible: ${JSON.stringify(result)}`); assert.match(`${result.htmlOverflowY} ${result.bodyOverflowY}`, /hidden/, `${label} disables surrounding scroll`); }
