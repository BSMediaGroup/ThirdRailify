...
test("Account V2 routes, saved addresses, orders, Cart, and Checkout are responsive and truthful", async (t) => {
  ...
  for (const [width, height] of [[1920,1080],[1440,900],[1024,768],[768,1024],[430,932],[390,844]]) {
    ...
    if (width === 1440) {
      ...
      await page.goto(`${ORIGIN}/account/delivery`); await page.getByRole("heading", { level: 1, name: "Delivery addresses" }).waitFor(); assert.match(await page.locator(".address-card").innerText(), /London, Ontario, Canada/); await page.getByRole("button", { name: "Add address" }).click(); await page.getByRole("dialog", { name: "Add a destination" }).waitFor(); assert.equal(await page.getByLabel("Country").inputValue(), "CA"); await page.getByLabel("Province / territory").selectOption("ON"); await page.getByLabel("Country").selectOption("US"); assert.equal(await page.getByLabel("State / territory").inputValue(), ""); await page.getByLabel("State / territory").selectOption("NY"); await page.getByLabel("Country").selectOption("CA"); assert.equal(await page.getByLabel("Province / territory").inputValue(), ""); await page.getByLabel("Province / territory").selectOption("ON"); await page.screenshot({ path: `${RESULTS}/account-address-editor-1440x900.png`, fullPage: true }); await page.getByRole("button", { name: "Close address editor" }).click();
  ...
  for (const [width,height] of [[1920,1080],[1440,900],[1024,768],[768,1024],[430,932],[390,844]]) {
    const { context,page,errors } = await fixturePage(browser,width,height);
    await page.goto(`${ORIGIN}/account/delivery`); await page.getByRole("heading", { level: 1, name: "Delivery addresses" }).waitFor(); await assertPage(page,"/account/delivery"); assert.equal(await page.locator(".address-card.is-default").count(),1); assert.match(await page.locator(".address-card").innerText(), /London, Ontario, Canada/); await page.screenshot({ path: `${RESULTS}/account-delivery-${width}x${height}.png`, fullPage:true }); await page.getByRole("button", { name: "Add address" }).click(); await page.getByRole("dialog", { name: "Add a destination" }).waitFor(); assert.equal(await page.getByLabel("Country").inputValue(), "CA"); if ([1440,390].includes(width)) await checkLocalityAutocomplete(page, `${RESULTS}/account-locality-${width}.png`); await page.screenshot({ path: `${RESULTS}/account-address-editor-${width}x${height}.png`, fullPage:true }); await page.getByRole("button", { name: "Close address editor" }).click();
  ...
  for (const [width,height] of [[1920,1080],[1440,900],[1024,768],[768,1024],[430,932],[390,844]]) {
    const { context,page,errors } = await fixturePage(browser,width,height,true); await page.goto(`${ORIGIN}/checkout`); await page.getByRole("heading", { level:1, name:"Delivery & checkout." }).waitFor(); const home = page.getByRole("radio", { name:/Home/ }); await home.waitFor(); assert.equal(await home.getAttribute("aria-checked"),"true"); assert.equal(await page.getByLabel("Recipient name").inputValue(),"Ada Rail"); assert.match(await home.innerText(), /London, Ontario, Canada/); assert.equal(await page.getByLabel("Customer email").isEditable(),false); await page.locator(".paypal-payment.is-unavailable").getByText("PayPal unavailable", { exact:true }).waitFor(); assert.match(await page.locator(".checkout-gate-message").innerText(),/currently unavailable/); assert.equal(await page.locator('input[name="card-number"]').count(),0); await assertPage(page,"/checkout"); await page.screenshot({ path:`${RESULTS}/checkout-${width}x${height}.png`, fullPage:true }); await page.getByRole("radio", { name: /Add another address/ }).click(); assert.equal(await page.getByLabel("Country").inputValue(), "CA"); assert.equal(await page.getByLabel("Province / territory").inputValue(), ""); if ([1440,390].includes(width)) await checkLocalityAutocomplete(page, `${RESULTS}/signed-in-locality-${width}.png`); await page.screenshot({ path:`${RESULTS}/checkout-add-address-${width}x${height}.png`, fullPage:true }); assert.deepEqual(errors,[]); await context.close();
  }
});
