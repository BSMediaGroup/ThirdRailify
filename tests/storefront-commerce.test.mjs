...
test("dedicated cart reuses CartProvider identity, stays CAD-only, and preserves the legacy redirect", async () => {
  ...
  assert.match(page, /useCart\(\)/); for (const label of ["Line total", "Subtotal", "Continue shopping", "Clear cart", "Review delivery readiness"]) assert.match(page, new RegExp(label, "i"));
