...
export async function onRequest(context) {
  ...
  try {
    ...
    if (request.method === "GET") return proxyRead(request, env, path, context.data?.wheelsFetch || fetch);
    if (!new Set(["POST", "PUT"]).has(request.method)) throw failure(405, "method_not_allowed", "This wheel method is not allowed.");
