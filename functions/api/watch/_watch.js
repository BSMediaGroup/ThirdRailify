...
export const WATCH_UPCOMING_CHECKPOINT_SECONDS = 150;
export const WATCH_UPCOMING_MIN_CHECKPOINT_SECONDS = 150;
export const WATCH_INACTIVE_CHECKPOINT_SECONDS = 600;
...
export function watchCheckpointSeconds(snapshot, env) {
  if (snapshot.liveNow.length) {
    return checkpointSeconds(
      env.THIRDRAILIFY_BROADCAST_LIVE_CHECKPOINT_SECONDS
  ...
  if (snapshot.upcoming) {
    return checkpointSeconds(
      env.THIRDRAILIFY_BROADCAST_UPCOMING_CHECKPOINT_SECONDS
  ...
  return checkpointSeconds(
    env.THIRDRAILIFY_BROADCAST_INACTIVE_CHECKPOINT_SECONDS
