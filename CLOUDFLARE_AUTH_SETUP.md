...
The Public site is not an account authority. `ThirdRailify-Admin` owns the D1 schema, credentials, verification/reset email, OAuth callbacks, abuse protection, audit, and roles. Public owns only its account UI plus same-origin session, one-time handoff, and logout Functions.
...
For staging, keep `THIRDRAILIFY_AUTH_COOKIE_DOMAIN` empty, use `https://thirdrailify.pages.dev` and `https://thirdrailify-admin.pages.dev` as the exact origins, and set the safe Vite build value `VITE_THIRDRAILIFY_ADMIN_ORIGIN=https://thirdrailify-admin.pages.dev`. Public must never receive Admin passwords, provider client secrets, the Turnstile secret, the rate-limit secret, or the Resend key.
